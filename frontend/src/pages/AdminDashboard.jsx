...
const T = {
  ...
  border: 'rgba(0,153,204,0.12)', cyan: '#0099cc', cyanDim: 'rgba(0,153,204,0.12)',
  text: '#f1f5f9', muted: '#64748b', sub: '#94a3b8',
  green: '#10b981', red: '#ef4444', amber: '#f59e0b', purple: '#0099cc',
};
...
const AdminDashboard = () => {
  ...
  const loadStats = useCallback(async () => {
    try {
      ...
      const s = d.stats || {};
      setStats({
        totalPatients: s.totalPatients || 0, activeDoctors: s.verifiedDoctors || 0,
        subscriptions: s.payingPatients || 0,
        aiAccuracy: s.totalPredictions > 0 ? +(s.approvedPredictions / s.totalPredictions * 100).toFixed(1) : 0,
        verifiedDoctors: s.verifiedDoctors || 0, provisionalDoctors: s.pendingDoctors || 0,
        activeSubscriptions: s.payingPatients || 0, expiredSubscriptions: s.expiredPatients || 0,
        trialSubscriptions: s.onTrial || 0, totalPredictions: s.totalPredictions || 0,
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 16 }}>
        <SectionCard title="📈 Subscription & Revenue Growth — Real Data">
          {analyticsLoading ? <ChartLoader /> : !hasChartItems(subscriptionGrowth) ? <EmptyChart msg="Could not load data — check backend." /> : (
            <ResponsiveContainer width="100%" height={260}>
              <AreaChart data={subscriptionGrowth}>
                <defs>
                  <linearGradient id="gSubs" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor={T.cyan} stopOpacity={0.3} /><stop offset="95%" stopColor={T.cyan} stopOpacity={0} /></linearGradient>
                  <linearGradient id="gRev" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor={T.green} stopOpacity={0.3} /><stop offset="95%" stopColor={T.green} stopOpacity={0} /></linearGradient>
                </defs>
        ...
        <SectionCard title="🤖 AI Queries / Day (Last 30d)">
          {analyticsLoading ? <ChartLoader /> : !hasChartItems(aiUsageData) ? <EmptyChart msg="Could not load AI usage data." /> : (
            <ResponsiveContainer width="100%" height={260}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
        <SectionCard title="👥 User Distribution — Real Data">
          {analyticsLoading ? <ChartLoader /> : !hasChartItems(userDist) ? <EmptyChart msg="No users registered yet" /> : (
            <ResponsiveContainer width="100%" height={260}>
              <PieChart>
        ...
        <SectionCard title="🎯 AI Approval Rate Trend — Real Data">
          {analyticsLoading ? <ChartLoader /> : !hasChartItems(accuracyTrend) ? <EmptyChart msg="Could not load trend data." /> : (
            <ResponsiveContainer width="100%" height={260}>
      ...
      <Card style={{ background: 'linear-gradient(135deg,rgba(16,185,129,0.08),rgba(0,153,204,0.06))', border: `1px solid ${T.green}30` }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            <div style={{ width: 44, height: 44, borderRadius: 12, background: `${T.green}18`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><UserCheck size={22} color={T.green} /></div>
            <div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 24, flexWrap: 'wrap' }}>
            <div style={{ flex: '1 1 260px', minWidth: 240 }}>
              <ResponsiveContainer width="100%" height={280}>
                <PieChart>
                  <Pie data={subDistData} cx="50%" cy="50%" outerRadius={105} dataKey="value"
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      <SectionCard title="👤 Admin Profile">
        <div style={{ display: 'flex', alignItems: 'center', gap: 20, marginBottom: 24 }}>
          <div style={{ position: 'relative', flexShrink: 0 }}>
            ...
            <button onClick={() => profileInputRef.current?.click()} style={{ position: 'absolute', bottom: 0, right: 0, width: 28, height: 28, borderRadius: '50%', background: T.cyan, border: `2px solid ${T.surface}`, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 14 }}>
          <div>
            ...
            <input type="text" value={user.name} onChange={e => setUser(u => ({ ...u, name: e.target.value }))} style={inputStyle} onFocus={e => e.target.style.borderColor = T.cyan} onBlur={e => e.target.style.borderColor = T.border} />
          </div>
          <div>
            <label style={labelStyle}>Email Address</label>
            <input type="email" value={user.email} onChange={e => setUser(u => ({ ...u, email: e.target.value }))} style={inputStyle} onFocus={e => e.target.style.borderColor = T.cyan} onBlur={e => e.target.style.borderColor = T.border} />
          </div>
        </div>
      ...
      <SectionCard title="⚙️ System Configuration">
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 20 }}>
          {[{ key: 'language', label: 'Language', options: ['English', 'Spanish', 'French', 'German', 'Hindi'] }, { key: 'timezone', label: 'Timezone', options: ['UTC', 'EST', 'PST', 'IST', 'CET'] }].map(s => (
            <div key={s.key}>
              <label style={labelStyle}>{s.label}</label>
              <select value={themeConfig[s.key]} onChange={e => handleSettingChange(s.key, e.target.value)} style={{ ...inputStyle, cursor: 'pointer' }}>
        ...
        <div style={{ paddingTop: 16, borderTop: `1px solid ${T.border}` }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {[
              ...
              <div key={item.key} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 16px', borderRadius: 10, background: T.surface, border: `1px solid ${T.border}`, cursor: 'pointer' }} onClick={() => handleSettingChange(item.key, !themeConfig[item.key])}>
                <div>
        ...
        <div style={{ maxWidth: 500 }}>
          ...
          {[
            ...
            <div key={field} style={{ marginBottom: 14 }}>
              ...
              <input type="password" placeholder={placeholder} value={pwForm[field]}
                onChange={e => setPwForm(f => ({ ...f, [field]: e.target.value }))}
                onKeyDown={e => e.key === 'Enter' && handlePasswordChange()}
                style={{ ...inputStyle, borderColor: field === 'confirm' && pwForm.confirm && pwForm.newPw !== pwForm.confirm ? T.red : T.border }}
                onFocus={e => e.target.style.borderColor = T.cyan}
                onBlur={e => e.target.style.borderColor = (field === 'confirm' && pwForm.confirm && pwForm.newPw !== pwForm.confirm) ? T.red : T.border}
              />
              {field === 'confirm' && pwForm.confirm && pwForm.newPw !== pwForm.confirm && <div style={{ fontSize: 11, color: T.red, marginTop: 4 }}>Passwords do not match</div>}
              {field === 'confirm' && pwForm.confirm && pwForm.newPw === pwForm.confirm && pwForm.confirm.length > 0 && (
                <div style={{ fontSize: 11, color: T.green, marginTop: 4, display: 'flex', alignItems: 'center', gap: 4 }}><Check size={11} /> Passwords match</div>
              )}
            </div>
          ))}
    ...
    <div style={{ minHeight: '100vh', background: T.bg, fontFamily: "'Inter', system-ui, sans-serif" }}>
      <nav style={{ position: 'fixed', top: 0, left: 0, right: 0, height: 60, zIndex: 50, background: `${T.surface}ee`, backdropFilter: 'blur(12px)', borderBottom: `1px solid ${T.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 20px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <div ref={notifRef} style={{ position: 'relative' }}>
            <button onClick={() => { const opening = !notifOpen; setNotifOpen(opening); if (opening) { loadNotifications(); if (unreadCount > 0) markNotificationsRead(); } }}
              ...
              <div style={{ position: 'absolute', top: 'calc(100% + 8px)', right: 0, width: 360, maxHeight: 460, background: T.card, border: `1px solid ${T.border}`, borderRadius: 14, boxShadow: '0 12px 48px rgba(0,0,0,0.7)', overflow: 'hidden', display: 'flex', flexDirection: 'column', zIndex: 200 }}>
                <div style={{ padding: '14px 18px', borderBottom: `1px solid ${T.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <button onClick={loadNotifications} style={{ background: 'none', border: 'none', cursor: 'pointer', color: T.muted, display: 'flex', padding: 4 }}><RefreshCw size={13} /></button>
                    {notifications.some(n => !n.is_read) && (
                      <button onClick={markNotificationsRead} style={{ fontSize: 11, color: T.cyan, background: 'none', border: 'none', cursor: 'pointer', fontWeight: 600, padding: '2px 4px' }}>Mark all read</button>
                    )}
                  </div>
                </div>
                <div style={{ overflowY: 'auto', flex: 1 }}>
                  {notifLoading ? (
                    <div style={{ padding: 40, textAlign: 'center' }}><RefreshCw size={22} color={T.cyan} style={{ animation: 'spin 1s linear infinite' }} /><div style={{ color: T.muted, fontSize: 12, marginTop: 8 }}>Loading…</div></div>
      ...
      <aside style={{ position: 'fixed', left: 0, top: 60, bottom: 0, width: sidebarOpen ? 240 : 0, overflow: 'hidden', background: T.surface, borderRight: `1px solid ${T.border}`, display: 'flex', flexDirection: 'column', transition: 'width 0.25s ease', zIndex: 40 }}>
        <div style={{ flex: 1, overflowY: 'auto', padding: '12px 10px' }}>
          {menuItems.map(item => {
            const Icon = item.icon; const active = activeMenu === item.id;
            return (
