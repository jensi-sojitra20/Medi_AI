  ...
  useEffect(() => {
    ...
    const loadAll = async () => {
      ...
      try {
        ...
        setPd({ name: pt.name || '', age: pt.age || '', gender: pt.gender || '', email: pt.email || '', phone: pt.phone || '', address: pt.address || '', bloodGroup: pt.blood_group || '', allergies: '', chronicConditions: '', profilePic: pt.profile_image ? `${import.meta.env.VITE_API_URL || 'http://localhost:8000'}/${pt.profile_image}` : null, height_cm: pt.height_cm || '', weight_kg: pt.weight_kg || '', bmi: pt.bmi || null });
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20, paddingBottom: 80 }}>
      ...
      <Modal isOpen={recordViewModal} onClose={() => setRecordViewModal(false)} title={`📋 ${viewingRecord?.title || 'Medical Record'}`}>
        {viewingRecord && (() => {
          ...
          const serverUrl = viewingRecord.file_path ? `${import.meta.env.VITE_API_URL || 'http://localhost:8000'}/${viewingRecord.file_path}` : null;
